...
Создает нового агента для пользователя через AMS.
...
- `agent_name` - название агента (опционально)
- `version` - конкретная версия шаблона (опционально)
- `use_latest` - использовать последнюю версию (по умолчанию true)
- `variables` - переменные шаблона (опционально)
